...
const routines = require('./routines/routines.js');
...
const moduler = module => new buddy.Module({
  debug: module.debug,
...
const projecter = project => new buddy.Project({
  index: project.index,
 ...
 */
// starts the client
cli.main((args, _options) => {
  // init options
  const options = _options || {};
  // loads configuration file
  const path = `/m-forge.${options.env}.json`;
  // load configuration
  configuration(path).load().then((config) => {
    // add modules in options object
    options.configuration = config;
    // create modules array
    const projectModules = [];
    // for each module in config file
    for (const module of options.configuration.modules) {
      // create a new module
      const newModule = moduler({
        debug: !options.min,
        location: module,
        destination: options.configuration.bundles,
      });
      // push module to modules array
      projectModules.push(newModule);
      // build project
    }
    // add modules in options object
    options.modules = projectModules;
    // create project
    const mForgeProject = projecter({
      starter: options.configuration.starter,
      constants: options.configuration.constants,
      index: options.configuration.index,
      modules: options.modules,
    });
    // add project in options object
    options.project = mForgeProject;
    // actions
    const action = args[0];
    // execute action
    if (typeof routines[action] !== 'undefined') {
      routines[action](options, (routinesMsg) => {
        console.log(routinesMsg.message);
      });
    } else {
